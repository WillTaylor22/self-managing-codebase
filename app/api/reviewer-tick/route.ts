...
export async function GET(req: Request) {
  ...
  if (
    ...
    return Response.json({ skipped: 'reviewer env not configured' });
  }
  ...
  const session = await client.beta.sessions.create({
    ...
    title: `reviewer-tick ${new Date().toISOString()}`,
  });
    ...
    events: [
      {
        ...
        content: [
          {
            type: 'text',
            text: 'Wake up. Scan open PRs. Review any that lack an AGENT_REVIEW: comment, per your system prompt. Then stop.',
          },
        ],
      },
    ],
