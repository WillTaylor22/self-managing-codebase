...
const REDIRECT = 'http://localhost:8767/callback';
const AUTH = 'https://mcp.sentry.dev/oauth/authorize';
const TOKEN = 'https://mcp.sentry.dev/oauth/token';
const REGISTER = 'https://mcp.sentry.dev/oauth/register';
const SCOPE = 'org:read project:write team:write event:write';
...
const server = createServer(async (req, res) => {
  ...
  res.writeHead(200, { 'Content-Type': 'text/html' }).end(
    '<h1>Sentry connected ✓</h1><p>You can close this tab.</p>',
  );

  console.log('\nAdd these to ai-manager/.env:\n');
  console.log(`SENTRY_MCP_CLIENT_ID=${reg.client_id}`);
  console.log(`SENTRY_MCP_ACCESS_TOKEN=${tok.access_token}`);
  console.log(`SENTRY_MCP_REFRESH_TOKEN=${tok.refresh_token ?? ''}`);
  console.log(`SENTRY_MCP_EXPIRES_AT=${expiresAt}`);
...
server.listen(8767, () => {
  console.log(`\nOpening Sentry authorize URL:\n  ${authUrl.toString()}\n`);
  console.log('Listening on http://localhost:8767 for the OAuth callback...');
