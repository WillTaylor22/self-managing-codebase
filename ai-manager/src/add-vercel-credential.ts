...
const cred = await client.beta.vaults.credentials.create(VAULT_ID, {
  ...
  auth: {
    ...
    access_token: required('VERCEL_MCP_ACCESS_TOKEN'),
    expires_at: required('VERCEL_MCP_EXPIRES_AT'),
    refresh: {
      refresh_token: required('VERCEL_MCP_REFRESH_TOKEN'),
      client_id: required('VERCEL_MCP_CLIENT_ID'),
